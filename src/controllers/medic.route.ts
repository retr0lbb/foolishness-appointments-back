...
router.post("/", async (req, res) => {
	...
	try {
		await conn.query(
			"INSERT INTO doctors(name, phone, crm) values (?, ?, ?);",
			[name, phone, crm],
		);
...
router.get("/", async (req, res) => {
	...
	try {
      ...
      SELECT 
        ...
        d.name, 
        d.phone,
        GROUP_CONCAT(ds.specialization SEPARATOR ', ') AS specializations,
        GROUP_CONCAT(ds.RQE SEPARATOR ', ') AS RQEs
      FROM doctors d
    ...
    `;
		...
		sql += " GROUP BY d.id, d.crm, d.name, d.phone";
...
router.get("/:id", async (req, res) => {
	...
	try {
		const [medic, _] = await conn.query<Doctor[]>(
		...
		if (medic.length >= 1) {
			return res
				.status(500)
				.send("Duplicated Keys Found in db, please warn db manager");
		}
...
router.delete("/:id", async (req, res) => {
	...
	try {
		...
		await conn.query("DELETE from doctors WHERE BIN_TO_UUID(id) = ?", [
			req.params.id,
		]);
...
router.put("/:id", async (req, res) => {
	...
	try {
		...
		if (medic.length > 1) {
			...
			return res
				.status(500)
				.send("Duplicated Keys found, contact database admin");
		}
